/**
 * patient-diary-list.js - 환자 일기 목록 페이지 JavaScript
 */
...
(function() {
    ...
    async function loadPatientList() {
        ...
        try {
            ...
            if (memberSelect) {
                memberSelect.innerHTML = '<option value="">-- 환자 선택 --</option>';
                ...
                memberSelect.addEventListener('change', function() {
                    ...
                    if (selectedPatientId) {
                        ...
                        $diaryListContainer.innerHTML = '<p class="text-center text-muted mt-5">환자를 선택해주세요.</p>';
    ...
    async function loadDiaries(page = 0) {
        if (!selectedPatientId || !$diaryListContainer) {
            $diaryListContainer.innerHTML = '<p class="text-center text-muted mt-5">환자를 선택해주세요.</p>';
            return;
        }
        ...
        const params = new URLSearchParams({ page: page, size: DIARY_PAGE_SIZE });
        const bodyData = { year, month, day, keyword };

        const url = `/api/diaries/list/${selectedPatientId}?${params.toString()}`;
    ...
    function renderDiaryList(diaries) {
        ...
        const html = diaries.map(diary => {
            ...
            const moods = [
                diary.anxietyRatio, diary.sadnessRatio, diary.joyRatio, diary.angerRatio,
                diary.regretRatio, diary.hopeRatio, diary.tirednessRatio, diary.depressionRatio, diary.neutralityRatio
            ];
