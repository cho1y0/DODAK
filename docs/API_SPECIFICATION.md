# Dodak API 명세서

## 문서 정보

| 항목 | 내용 |
|------|------|
| 프로젝트명 | Dodak (AI 기반 감정 일기 & 심리 케어 플랫폼) |
| 문서 버전 | 1.0 |
| 작성일 | 2026-02-03 |

---

## 1. API 개요

### 1.1 서버 구성

| 서버 | URL | 설명 |
|------|-----|------|
| Spring Boot | http://192.168.0.x:8083 | 메인 웹 서버 |
| FastAPI AI | http://192.168.0.20:8000 | AI 분석 서버 |

### 1.2 인증 방식

- **Spring Boot**: Spring Security 세션 기반 인증
- **FastAPI**: 별도 인증 없음 (내부 네트워크 접근)

---

## 2. FastAPI AI Server API

### 2.1 서버 상태 확인

```
GET /
```

**응답**
```json
{
  "message": "Integrated Server (Diary, Chatbot, Crawler) is running 🚀 - FastAPI"
}
```

---

### 2.2 일기 감정 분석

```
GET /diary?s={일기내용}
```

**파라미터**

| 이름 | 타입 | 필수 | 설명 |
|------|------|------|------|
| s | string | O | URL 인코딩된 일기 텍스트 |

**요청 예시**
```
GET /diary?s=%EC%98%A4%EB%8A%98%20%EB%84%88%EB%AC%B4%20%ED%9E%98%EB%93%A4%EC%97%88%EB%8B%A4
```

**응답**
```json
{
  "joy": 0.0,
  "hope": 0.0,
  "neutrality": 0.0,
  "anger": 0.0,
  "sadness": 50.0,
  "anxiety": 0.0,
  "tiredness": 50.0,
  "regret": 0.0,
  "depression": 0.0
}
```

**감정 설명**

| 필드 | 설명 | 범위 |
|------|------|------|
| joy | 기쁨 비율 | 0.0 ~ 100.0 |
| hope | 희망 비율 | 0.0 ~ 100.0 |
| neutrality | 중립 비율 | 0.0 ~ 100.0 |
| anger | 분노 비율 | 0.0 ~ 100.0 |
| sadness | 슬픔 비율 | 0.0 ~ 100.0 |
| anxiety | 불안 비율 | 0.0 ~ 100.0 |
| tiredness | 피로 비율 | 0.0 ~ 100.0 |
| regret | 후회 비율 | 0.0 ~ 100.0 |
| depression | 우울증 위험도 | 0.0 ~ 100.0 |

**분석 알고리즘**
1. 입력 텍스트 URL 디코딩
2. Regex 기반 문장 분리
3. 문장별 감정 키워드 분석
4. 긍정/부정/중립 1차 분류
5. 8가지 세부 감정 분류
6. 우울증 키워드 검출
7. 비율 계산 및 반환

---

### 2.3 Simple 챗봇 (KoGPT2)

```
GET /chatbot/g?s={메시지}
```

**파라미터**

| 이름 | 타입 | 필수 | 설명 |
|------|------|------|------|
| s | string | X | 사용자 입력 메시지 |

**요청 예시**
```
GET /chatbot/g?s=안녕하세요
```

**응답**
```json
{
  "answer": "안녕하세요. 저는 당신의 이야기를 들어줄 수 있는 챗봇입니다."
}
```

**빈 입력 시 응답**
```json
{
  "answer": "듣고 있어요. 더 말씀해주세요~ (끄덕끄덕)"
}
```

---

### 2.4 Complex 챗봇 (KoBERT)

```
GET /chatbot/b?s={메시지}
```

**파라미터**

| 이름 | 타입 | 필수 | 설명 |
|------|------|------|------|
| s | string | X | 사용자 입력 메시지 |

**요청 예시**
```
GET /chatbot/b?s=요즘 너무 우울해요
```

**응답**
```json
{
  "answer": "현재 많이 힘들어 보이세요. 전문 상담사와 연결해 드릴까요?"
}
```

**특수 응답 케이스**

| 감지 키워드 | 응답 유형 |
|-------------|-----------|
| 화가, 분노 | 분노 조절 가이드 |
| 우울, 죽고 | 전문 상담 연결 안내 |
| (기타) | 일반 공감 응답 |

---

### 2.5 YouTube 음악 검색

```
POST /youtube/search
```

**요청 헤더**
```
Content-Type: application/json
```

**요청 본문**
```json
{
  "keyword": "슬픔"
}
```

**지원 키워드**

| 키워드 | 검색 쿼리 |
|--------|-----------|
| 분노 | 분노 가라앉히는 음악 |
| 불안 | 불안할때 듣는 노래 |
| 슬픔 | 슬플때 듣는 노래 |
| 기쁨 | 기쁠때 듣는 노래 |
| 후회 | 후회할때 듣는노래 |
| 희망 | 희망찬 브금 |
| 중립 | 머릿속이 복잡할때 듣는 노래 |
| 피로 | 지치고 힘들때 듣는 노래 |
| 우울 | 우울함을 달래줄 노래 |

**응답**
```json
{
  "keyword": "슬픔",
  "results": [
    {
      "name": "슬플때 듣는 노래 모음",
      "thumnail": "https://i.ytimg.com/vi/xxx/hqdefault.jpg",
      "link": "https://www.youtube.com/watch?v=xxx",
      "viewCount": "100만회",
      "videoDate": "1년 전",
      "videoAuthor": "채널명",
      "videoAuthorImg": "https://yt3.ggpht.com/xxx",
      "videoDescript": "영상 설명..."
    }
  ],
  "time": "2026-02-03 14:30:00"
}
```

**응답 필드**

| 필드 | 타입 | 설명 |
|------|------|------|
| keyword | string | 요청 키워드 |
| results | array | 영상 목록 (최대 10개) |
| time | string | 검색 시각 |

**results 배열 항목**

| 필드 | 타입 | 설명 |
|------|------|------|
| name | string | 영상 제목 |
| thumnail | string | 썸네일 URL |
| link | string | YouTube 링크 |
| viewCount | string | 조회수 |
| videoDate | string | 업로드 시기 |
| videoAuthor | string | 채널명 |
| videoAuthorImg | string | 채널 이미지 URL |
| videoDescript | string | 영상 설명 |

**에러 응답**
```json
{
  "detail": "검색 결과 없음"
}
```
HTTP Status: 404

---

## 3. Spring Boot REST API

### 3.1 회원 API

#### 3.1.1 회원 정보 조회

```
GET /api/member/{id}
```

**응답**
```json
{
  "id": 1,
  "userId": "user123",
  "name": "홍길동",
  "email": "hong@example.com",
  "phone": "010-1234-5678",
  "zipCode": "12345",
  "addr1": "서울시 강남구",
  "addr2": "테헤란로 123",
  "role": "USER",
  "profileImg": "/uploads/profile/xxx.jpg",
  "joinedAt": "2026-01-15T10:30:00"
}
```

#### 3.1.2 회원 정보 수정

```
PUT /api/member/{id}
```

**요청**
```json
{
  "password": "newPassword123",
  "name": "홍길동",
  "phone": "010-9876-5432",
  "zipCode": "54321",
  "addr1": "서울시 서초구",
  "addr2": "서초대로 456",
  "profileImg": "/uploads/profile/new.jpg"
}
```

---

### 3.2 일기 API

#### 3.2.1 일기 목록 조회

```
GET /api/diary?memberId={memberId}
```

**응답**
```json
[
  {
    "diaryIdx": 1,
    "diaryTitle": "오늘의 일기",
    "diaryContent": "오늘은 좋은 하루였다...",
    "file1": "/uploads/diary/xxx.jpg",
    "file2": null,
    "file3": null,
    "createdAt": "2026-02-03T10:00:00",
    "analysis": {
      "joyRatio": 50.0,
      "sadnessRatio": 10.0
    }
  }
]
```

#### 3.2.2 일기 작성

```
POST /api/diary
```

**요청 (multipart/form-data)**

| 필드 | 타입 | 필수 | 설명 |
|------|------|------|------|
| memberId | int | O | 회원 ID |
| diaryTitle | string | O | 제목 |
| diaryContent | string | O | 내용 |
| file1 | file | X | 첨부파일 1 |
| file2 | file | X | 첨부파일 2 |
| file3 | file | X | 첨부파일 3 |

**응답**
```json
{
  "diaryIdx": 10,
  "message": "일기가 저장되었습니다."
}
```

#### 3.2.3 일기 수정

```
PUT /api/diary/{diaryIdx}
```

#### 3.2.4 일기 삭제

```
DELETE /api/diary/{diaryIdx}
```

---

### 3.3 분석 API

#### 3.3.1 분석 결과 조회

```
GET /api/analysis?diaryIdx={diaryIdx}
```

**응답**
```json
{
  "analysisIdx": 1,
  "diaryIdx": 10,
  "modelName": "skt/kobert-base-v1",
  "anxietyRatio": 10.0,
  "sadnessRatio": 20.0,
  "joyRatio": 25.5,
  "angerRatio": 5.0,
  "regretRatio": 6.0,
  "hopeRatio": 10.0,
  "neutralityRatio": 15.0,
  "tirednessRatio": 8.5,
  "depressionRatio": 15.0,
  "createdAt": "2026-02-03T10:05:00"
}
```

#### 3.3.2 분석 결과 저장

```
POST /api/analysis
```

**요청**
```json
{
  "diaryIdx": 10,
  "modelName": "skt/kobert-base-v1",
  "anxiety": 10.0,
  "sadness": 20.0,
  "joy": 25.5,
  "anger": 5.0,
  "regret": 6.0,
  "hope": 10.0,
  "neutrality": 15.0,
  "tiredness": 8.5,
  "depression": 15.0
}
```

---

### 3.4 병원 API

#### 3.4.1 병원 목록 조회

```
GET /api/hospital
```

#### 3.4.2 병원 등록

```
POST /api/hospital
```

**요청**
```json
{
  "hospName": "마음건강 병원",
  "zipCode": "12345",
  "addr1": "서울시 강남구",
  "addr2": "테헤란로 100",
  "tel": "02-1234-5678"
}
```

---

### 3.5 의사 API

#### 3.5.1 의사 목록 조회

```
GET /api/doctor
```

#### 3.5.2 의사 등록

```
POST /api/doctor
```

---

### 3.6 예약 API

#### 3.6.1 예약 목록 조회

```
GET /api/arrange?memberId={memberId}
```

#### 3.6.2 예약 신청

```
POST /api/arrange
```

**요청**
```json
{
  "memberId": 1,
  "docIdx": 5,
  "arrDate": "2026-02-10",
  "arrTime": "14:00",
  "memo": "첫 상담입니다"
}
```

#### 3.6.3 예약 상태 변경

```
PUT /api/arrange/{arrIdx}/status
```

**요청**
```json
{
  "status": "CONFIRMED"
}
```

**상태값**

| 상태 | 설명 |
|------|------|
| PENDING | 대기 중 |
| CONFIRMED | 확정 |
| CANCELLED | 취소 |
| COMPLETED | 완료 |

---

## 4. 에러 코드

### 4.1 HTTP 상태 코드

| 코드 | 설명 |
|------|------|
| 200 | 성공 |
| 201 | 생성 성공 |
| 400 | 잘못된 요청 |
| 401 | 인증 필요 |
| 403 | 권한 없음 |
| 404 | 리소스 없음 |
| 500 | 서버 오류 |

### 4.2 에러 응답 형식

```json
{
  "timestamp": "2026-02-03T10:00:00",
  "status": 400,
  "error": "Bad Request",
  "message": "유효하지 않은 입력값입니다.",
  "path": "/api/diary"
}
```

---

## 5. CORS 설정

### 5.1 허용 Origin

```python
origins = [
    "http://192.168.0.9:8083",
    "http://192.168.0.5:8083",
    "http://192.168.0.1:8083",
    "http://192.168.0.21:8083",
    "http://localhost:8083",
    "http://127.0.0.1:8083",
]
```

### 5.2 허용 설정

- 모든 HTTP 메서드 허용
- 모든 헤더 허용
- 자격 증명(쿠키) 허용

---

## 6. 관련 문서

| 문서 | 설명 |
|------|------|
| [FUNCTIONAL_SPECIFICATION.md](./FUNCTIONAL_SPECIFICATION.md) | 기능 명세서 |
| [TECH_STACK.md](./TECH_STACK.md) | 기술 스택 |
| [TEST_CASES.md](./TEST_CASES.md) | 테스트 케이스 |

---

## 7. 변경 이력

| 버전 | 날짜 | 변경 내용 | 작성자 |
|------|------|-----------|--------|
| 1.0 | 2026-02-03 | 최초 작성 | 박제연 |
| 1.1 | 2026-02-09 | 관련 문서 링크 추가, 최신화 | 박제연 |
